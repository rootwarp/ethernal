...
// ---------------------------------------------------------------------------
// F7-1 / FR-31 — eight architecture-§9 error paths on `validator recover`
// ---------------------------------------------------------------------------

/// Drive `validator recover` with a fixed mnemonic and a single
/// `--passphrase-file` path. Captures stdout + stderr (the log stream is stderr).
fn run_validator_recover_ks_file(out_label: &str, ks_path: &Path) -> Output {
    ...
    child.wait_with_output().expect("wait validator recover")
}
...
#[test]
fn validator_passphrase_file_not_found_exit2_no_leak() {
    let secrets = TempDir::new("f7-1-val-nf-s");
    // Distinctive path segment so a path-echo is unambiguous; content sentinel
    // would only appear if a wrong code path read a real secret.
    const SENTINEL: &str = "F71_VAL_NOTFOUND_SENTINEL_a3c9";
    let missing = secrets.join("missing-F71_VAL_NOTFOUND.pw");
    let path_str = missing.to_str().unwrap().to_owned();

    let out = run_validator_recover_ks_file("f7-1-val-nf", &missing);
...
#[cfg(unix)]
#[test]
fn validator_passphrase_file_permission_denied_0000_exit2_no_leak() {
    ...
    const SENTINEL: &str = "F71_VAL_PERM_DENIED_SENTINEL_b7e1";
    let secrets = TempDir::new("f7-1-val-perm-s");
    ...
    let out = run_validator_recover_ks_file("f7-1-val-perm", &ks_path);
...
#[test]
fn validator_passphrase_file_is_directory_fr14_not_os_error21() {
    let secrets = TempDir::new("f7-1-val-dir-s");
    // The passphrase "path" is a directory created under the secrets temp dir.
    ...
    // Distinctive sentinel that must never appear (no file contents to leak, but
    // guard against any accidental echo of a fixed string).
    const SENTINEL: &str = "F71_VAL_ISDIR_SENTINEL_c4d2";

    let out = run_validator_recover_ks_file("f7-1-val-dir", &dir_as_file);
    ...
    // FR-14: never the raw OS string from a late read failure.
    assert!(
...
#[test]
fn validator_passphrase_file_empty_exit2_no_leak() {
    const SENTINEL: &str = "F71_VAL_EMPTY_SENTINEL_d8f0";
    let secrets = TempDir::new("f7-1-val-empty-s");
    // Empty file: no payload. Sentinel is only for the absence assertion shape.
    let ks_path = secret_file(&secrets, "empty.pw", b"");
    let path_str = ks_path.to_str().unwrap().to_owned();

    let out = run_validator_recover_ks_file("f7-1-val-empty", &ks_path);
...
#[test]
fn validator_passphrase_file_multiline_exit2_line_count_no_leak() {
    const SENTINEL: &str = "F71_VAL_MULTI_SENTINEL_e1a7";
    let secrets = TempDir::new("f7-1-val-multi-s");
    let bytes = format!("{SENTINEL}\nsecond-line-must-not-leak\n");
    let ks_path = secret_file(&secrets, "multi.pw", bytes.as_bytes());
    let path_str = ks_path.to_str().unwrap().to_owned();

    let out = run_validator_recover_ks_file("f7-1-val-multi", &ks_path);
    ...
    // FR-9: residual is a line count, not content.
    assert!(
...
#[test]
fn validator_passphrase_file_cr_shapes_exit2_no_leak() {
    const SENTINEL: &str = "F71_VAL_CR_SENTINEL_f2b8";
    for (label, bytes) in [
        ("cr", format!("{SENTINEL}\r").into_bytes()),
        ("crlf", format!("{SENTINEL}\r\n").into_bytes()),
        ("crcrlf", format!("{SENTINEL}\r\r\n").into_bytes()),
    ] {
        let secrets = TempDir::new(&format!("f7-1-val-cr-s-{label}"));
        let ks_path = secret_file(&secrets, "cr.pw", &bytes);
        let path_str = ks_path.to_str().unwrap().to_owned();

        let out = run_validator_recover_ks_file(&format!("f7-1-val-cr-{label}"), &ks_path);
...
#[test]
fn validator_passphrase_file_oversize_4097_exit2_no_leak() {
    const SENTINEL: &str = "F71_VAL_OVERSIZE_SENTINEL_g3c1";
    let secrets = TempDir::new("f7-1-val-over-s");
    ...
    let out = run_validator_recover_ks_file("f7-1-val-over", &ks_path);
...
#[cfg(unix)]
#[test]
fn validator_passphrase_file_dev_zero_exit2() {
    // No file-byte sentinel: /dev/zero yields zeros. Assert exit 2 + non-empty msg.
    let out = run_validator_recover_ks_file("f7-1-val-zero", Path::new("/dev/zero"));
    let stdout = String::from_utf8_lossy(&out.stdout);
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert_eq!(
        out.status.code(),
    ...
    // Distinctive zero-payload must not be mis-rendered as a huge secret dump.
    assert!(
        stderr.len() < 4096,
...
#[test]
fn validator_passphrase_file_non_utf8_exit2_no_leak() {
    const SENTINEL: &str = "F71_VAL_NUTF8_SENTINEL_h4d9";
    let secrets = TempDir::new("f7-1-val-utf8-s");
    ...
    let out = run_validator_recover_ks_file("f7-1-val-utf8", &ks_path);
