// gen.go wires the "gen" subcommand: it composes the BLS/keystore/deposit
// internal packages into eth-deposit's deposit_data generator, formerly the
// standalone eth-deposit-gen binary.
package main
...
// but cfg.MainnetAck is false. The CLI gate in the gen Action catches this first
// for CLI callers; this sentinel protects non-CLI callers (integration tests,
// future programmatic APIs) and maps to exit code 2.
...
// genCommand returns the urfave/cli "gen" subcommand definition.
func genCommand() *ucli.Command {
	return cli.NewApp(runGen)
}
...
// genDeps holds the injectable dependencies for runGenWithDeps. In production
// these are filled with real implementations; in tests they can be replaced
// with fakes.
type genDeps struct {
...
// buildGenLogger constructs a *slog.Logger based on the verbose and jsonLogs flags.
...
func buildGenLogger(verbose, jsonLogs bool, w io.Writer) *slog.Logger {
...
func emitProgress(d genDeps, cfg cli.Config, done, total int) {
	if cfg.JSONLogs {
...
// productionGenDeps returns the genDeps wired with all real implementations.
// The logger field is intentionally set to a discarding logger here; runGen()
// overrides it with the cfg-configured logger before calling runGenWithDeps.
func productionGenDeps() genDeps {
	return genDeps{
		initBLS:          bls.Init,
...
// runGenWithDeps is the testable core of runGen. It accepts a genDeps struct so
// tests can inject fakes without touching the real BLS or keystore implementations.
// It follows the exact wiring order prescribed by Issue #25.
func runGenWithDeps(ctx context.Context, cfg cli.Config, d genDeps) error {
	log := d.logger
	...
	// skip the safety gate by constructing a Config directly. The gen Action fires
	// first for CLI callers and returns before reaching this point.
	...
	printGenSummary(d.summaryOut, path, sum, len(entries), cfg.Network)
	return nil
}

// runGen is the urfave/cli action function for the gen subcommand. It delegates
// to runGenWithDeps with the production dependency set. The writer and logger
// are configured here from cfg so that productionGenDeps() remains free of flag
// knowledge.
func runGen(ctx context.Context, cfg cli.Config) error {
	d := productionGenDeps()
	d.writer = pickWriter(cfg, os.Stdout)
	d.logger = buildGenLogger(cfg.Verbose, cfg.JSONLogs, os.Stderr)
	return runGenWithDeps(ctx, cfg, d)
}

// printGenSummary writes the success summary line to w.
...
func printGenSummary(w io.Writer, path, sha256hex string, n int, net network.Network) {
	display := path
	if display == "" {
		display = "<stdout>"
	}
